...
class DbConfig():
    ...
    def __init__(self):
        ...
        self.store_table = 'state'
        self.data_table = 'burlington'
    ...
    def insert_store_table(self, item):
        ...
        query = f'''INSERT INTO {self.store_table} (store_link, country, state) VALUES (%s, %s, %s)'''
        data = (
            item["store_link"],
            item["country"],
            item["state"]
    ...
    def update_store_status(self,c_status,search_status):
        qr = f'''update {self.store_table} set status = "{c_status}" where state = "{search_status}"'''
        print(qr)
        try:
            self.cur.execute(qr)
            self.con.commit()
