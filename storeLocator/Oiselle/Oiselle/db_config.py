...
class DbConfig():
    ...
    def __init__(self):
        ...
        self.store_table = 'usa_pincodes'
        self.data_table = 'oiselle'
    ...
    def update_store_status(self,c_status,lat_lng):
        qr = f'''update {self.store_table} set status = "{c_status}" where lat_lng = "{lat_lng}"'''
        print(qr)
        try:
            self.cur.execute(qr)
            self.con.commit()
